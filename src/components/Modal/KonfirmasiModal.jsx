...
const KonfirmasiModal = ({ 
  ...
  const getKategoriName = (id) => kategoriList.find(k => String(k.id_kategori) === String(id))?.nama_kategori || '-';
  const getUnitName = (id) => unitList.find(u => String(u.id_unit) === String(id))?.nama_unit || '-';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600">
            Mohon periksa kembali data Anda sebelum mengirim. Apakah data berikut sudah benar?
          </p>

          <div className="bg-blue-50 rounded-xl p-4 border border-blue-100 space-y-3">
            <div className="grid grid-cols-3 gap-2 text-sm">
              <span className="text-gray-500 font-medium">Judul Kasus</span>
              <span className="col-span-2 text-gray-800 font-semibold">{data.nama_kasus}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-sm">
              <span className="text-gray-500 font-medium">Kategori</span>
              <span className="col-span-2 text-gray-800">{getKategoriName(data.id_kategori)}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-sm">
              <span className="text-gray-500 font-medium">Unit Terkait</span>
              <span className="col-span-2 text-gray-800">{getUnitName(data.id_unit)}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-sm">
              <span className="text-gray-500 font-medium">Terlapor</span>
              <span className="col-span-2 text-gray-800">{data.subjek_pelaku || '-'}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-sm">
              <span className="text-gray-500 font-medium">Waktu</span>
              <span className="col-span-2 text-gray-800">
                {data.waktu_kejadian ? new Date(data.waktu_kejadian).toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' }) : '-'}
              </span>
            </div>
          ...
          <p className="text-xs text-gray-500 italic">
            * Dengan mengirim aduan ini, Anda menyatakan bahwa informasi yang diberikan adalah benar dan dapat dipertanggungjawabkan.
          </p>
        </div>
